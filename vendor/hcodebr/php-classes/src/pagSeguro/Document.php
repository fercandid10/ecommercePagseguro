...
class Document {
    ...
    public function __construct(string $type, string $value){
        ...
        if (!$value){
            throw new Exception("Informe o valor do ducmento");
    ...
    public static function isValidCPF($number):bool{
        ...
        for ($i = 0, $j = 10, $sum = 0; $i < 9; $i++, $j--) $sum += $number{$i} * $j;
        $rest = $sum % 11;
        if ($number{9} != ($rest < 2 ? 0 : 11 - $rest)) return false;

        for ($i = 0, $j = 11, $sum = 0; $i < 10; $i++, $j--) $sum += $number{$i} * $j;
        $rest = $sum % 11;
